...
def get_distance_professional(person_lat, person_lon, fence_coords):
    ...
    # 转换围栏坐标 (lat, lon) 
    fence_pts = [transformer.transform(float(lon), float(lat)) for lon, lat in fence_coords]
...
def analyze():
    ...
    for i, det in enumerate(detections):
        ...
        if loc_res:
            ...
            #fence = [(116.280359, 40.050871),(116.279826, 40.052784)]
            fence = [(116.280089, 40.051902),(116.279886, 40.052784)]
            print(loc_res['lat'])
            distance = get_distance_professional(loc_res['lat'], loc_res['lng'], fence)
